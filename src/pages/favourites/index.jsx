...
import { DelFav, GetFavs } from '../../api/favourites/favs'
...
function Card({ image, setImage }) {
	...
	const onRemove = async () => {
		const res = DelFav(image.imageid)
		res.then(() => {
			toast.success('Removed from favorites')
			const result = GetFavs(user?._id)
			result
				.then((res) => {
...
export default function Favorites() {
	...
	if (images.length > 0) {
		for (let i = 0; i < images.length; i++) {
			...
			if (i % 4 == 0)
				col1.push(<Card image={image} setImage={setImages} />)
			else if (i % 4 == 1)
				col2.push(<Card image={image} setImage={setImages} />)
			else if (i % 4 == 2)
				col3.push(<Card image={image} setImage={setImages} />)
			else if (i % 4 == 3)
				col4.push(<Card image={image} setImage={setImages} />)
		}
